...
import { CheckCircle2, FileImage, Loader2, Sparkles, Upload } from "lucide-react";
...
export function OrderForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userError || !userData.user) {
      setStatus({
        type: "error",
        message: "請先登入後再提交訂單，使用 Google 帳號即可開始定制。",
      });
      return;
    }
    ...
    try {
      ...
      setStatus({
        type: "success",
        message: "訂單已成功提交！我們將在 24 小時內與您確認細節。",
      });
      setForm(initialForm);
  ...
  return (
    <section id="plans" className="rounded-[32px] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-pink-500/10 backdrop-blur-xl sm:p-8 lg:p-10">
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          <p className="mb-2 inline-flex items-center gap-2 rounded-full border border-pink-400/30 bg-pink-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-pink-200">
            <Sparkles className="h-3.5 w-3.5" />
            需求提交
          </p>
          <h2 className="text-3xl font-black tracking-tight text-white">開始你的專屬角色訂製</h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6">
        <div className="grid gap-6 md:grid-cols-2">
          <label className="grid gap-2 text-sm font-medium text-slate-200">
            ...
            <input
              ...
              placeholder="例如：夜色織夢"
              className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white placeholder:text-slate-400 focus:border-pink-400 focus:outline-none"
            />
          </label>
          ...
          <label className="grid gap-2 text-sm font-medium text-slate-200">
            ...
            <select
              ...
              className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white focus:border-pink-400 focus:outline-none"
            >
        ...
        <label className="grid gap-2 text-sm font-medium text-slate-200">
          ...
          <div className="rounded-2xl border border-dashed border-white/15 bg-slate-950/70 p-4">
            <input
              ...
              className="block w-full text-sm text-slate-300 file:mr-4 file:rounded-full file:border-0 file:bg-pink-500/20 file:px-4 file:py-2 file:text-sm file:font-medium file:text-pink-200 hover:file:bg-pink-500/30"
            />
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <label className="grid gap-2 text-sm font-medium text-slate-200">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/70 px-3 py-2">
              ...
              />
              <span className="font-mono text-slate-300">{form.hairColor}</span>
            </div>
          </label>
          ...
          <label className="grid gap-2 text-sm font-medium text-slate-200">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/70 px-3 py-2">
              <input
                ...
                className="h-11 w-16 cursor-pointer rounded-lg border border-white/10 bg-transparent p-0"
              />
              <span className="font-mono text-slate-300">{form.eyeColor}</span>
            </div>
          </label>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-pink-500 via-violet-500 to-cyan-400 px-5 py-3.5 text-base font-semibold text-white shadow-lg shadow-pink-500/30 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70"
        >
          {isSubmitting ? (
            <>
